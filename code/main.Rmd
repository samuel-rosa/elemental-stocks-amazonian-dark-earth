...
# NOTE: must use car package version < 3.0-0
...
save(pointData, file = "../res/R/pointData.rda")
# load("../res/R/pointData.rda")
...
if (plot_labels) {
  p <- p + 
    shadowtext::geom_shadowtext(
      ggplot2::aes(label = "Solimões River", x = -60.233, y = -3.261, bg.color = "white"), 
      col = "black", size = 5) +
    shadowtext::geom_shadowtext(
      ggplot2::aes(label = "Caldeirão", x = lab[1], y = lab[2] + 0.0005, bg.color = "white"), 
      col = "black", size = 5)
}

# Save image
ggplot2::ggsave(file = "../res/fig/caldeirao.svg", plot = p, width = 14, height = 14, dpi = 1200)
...
map <- ggmap::get_googlemap(
  center = center, maptype = "terrain", color = "bw", 
...
if (plot_labels) {
  p <- p + 
    shadowtext::geom_shadowtext(
      ggplot2::aes(label = "Caldeirão", x = lab[1] + 0.17, y = lab[2], bg.color = "white"), 
      col = "black", size = 10) +
    shadowtext::geom_shadowtext(
      ggplot2::aes(label = "Amazonas State", x = lab[1] + 0.5, y = lab[2] - 0.25, bg.color = "white"), 
      col = "black", size = 10)
}
# Save regional-scale image
ggplot2::ggsave(file = "../res/fig/manaus.svg", plot = p, width = 7, height = 7, dpi = 1200)
...
center <- c(mean(location[c(1, 3)]), mean(location[c(2, 4)]))
map <- ggmap::get_googlemap(
  center = center, maptype = "terrain", color = "bw", zoom = 4, 
  style = c(feature = "all", element = "labels", visibility = "off"))
...
if (plot_labels) {
  p <- p + 
    shadowtext::geom_shadowtext(
      ggplot2::aes(label = "Amazonas\nState", x = lab[1], y = lab[2], bg.color = "white"), 
      col = "black", size = 10) +
    shadowtext::geom_shadowtext(
      ggplot2::aes(label = "BRAZIL", x = center[1], y = center[2], bg.color = "white"), 
      col = "black", size = 10)
}
# Save country-scale image
ggplot2::ggsave(file = "../res/fig/brasil.svg", plot = p, width = 7, height = 7, dpi = 1200)
png("../res/fig/brasil.png", width = 480 * 2, height = 480 * 2, res = 72 * 2)
p
dev.off()
rm(p, location, map, lab)
rm(boundary)
```
...
# save results
save(tooc_vario, tooc_lmc, file = "../res/R/tooc_vario.rda")
```
...
if (file.exists("../res/R/tooc_pred.rda")) {
  ...
  save(tooc_pred, file = "../res/R/tooc_pred.rda")
}
```
...
```{r}
if (file.exists("../res/R/tooc_ade.rda")) {
  load("../res/R/tooc_ade.rda")
} else {
...
# save
save(tooc_ade, file = "../res/R/tooc_ade.rda")
rm(tooc_sim, tooc_lim)
gc()
}
...
# save results
# save(camg_vario, camg_lmc, file = "../res/R/camg_vario.rda")
# load("../res/R/camg_vario.rda")
```
...
# save(camg_pred, file = "../res/R/camg_pred.rda")
```

### Spatial simulation

```{r}
if (file.exists("../res/R/camg_ade.rda")) {
  load("../res/R/camg_ade.rda")
} else {
# make spatial simulations
nsim <- 1000
t0 <- proc.time()
...
# save
save(camg_ade, file = "../res/R/camg_ade.rda")
rm(camg_sim, camg_lim)
gc()
}
...
# save results
# save(exph_vario, exph_lmc, file = "../res/R/exph_vario.rda")
# load("../res/R/exph_vario.rda")
```
...
# save(exph_pred, file = "../res/R/exph_pred.rda")
```

### Spatial simulation

```{r}
if (file.exists("../res/R/exph_ade.rda")) {
  load("../res/R/exph_ade.rda")
} else {
# make spatial simulations
nsim <- 1000
t0 <- proc.time()
...
# Save
save(exph_ade, file = "../res/R/exph_ade.rda")
rm(exph_sim, exph_lim)
gc()
}
...
write.csv(summary(pretic$pretic), file = "../res/tab/area.csv")
```
...
# save results
save(toca_vario, toca_lmc, file = "../res/R/toca_vario.rda")
```

### Spatial prediction

```{r}
if (file.exists("../res/R/toca_pred.rda")) {
  load("../res/R/toca_pred.rda")
} else {
# make spatial predictions
t0 <- proc.time()
...
save(toca_pred, file = "../res/R/toca_pred.rda")
}
```
...
# save results
save(toph_vario, toph_lmc, file = "../res/R/toph_vario.rda")
```

### Spatial predictions

```{r}
if (file.exists("../res/R/toph_pred.rda")) {
  load("../res/R/toph_pred.rda")
} else {
# make spatial predictions
t0 <- proc.time()
...
save(toph_pred, file = "../res/R/toph_pred.rda")
}
```
